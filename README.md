...
**开发入口统一为 `main`，当前为 LD-03A LocalDev，不是可用 VPN 发行版。** 本轮增加 WireGuard 参数编辑与凭据安全换绑；保留批量规则、搜索、结构导入及 Keychain 恢复。用户反馈 LD-02C 显示正常，只记为该布局的 USER_REPORTED 结果，不据此认定新参数窗口或 Keychain 已验收。最新结果见 [LD-03A 证据](docs/evidence/localdev-03a-parameters.md)；不需要历史补丁或 ZIP。
...
有未提交修改或分支分叉时保留工作并停止，不 reset、clean 或强制覆盖。无需先运行中间版本。窗口顶部应显示“本地开发模式：不接管网络”和 LD-03A。
...
源码更新不迁移草稿。旧 v1/v2 可以读取；确认结构导入至少使用 v2，首次凭据事务准备记录升级 v3，即使其后授权失败。旧程序会拒绝 v3，不手改版本、清空工作区或恢复旧 JSON 来回退。LD-03A 不新增工作区或凭据记录版本。
