...
当前状态：S0 IN PROGRESS（历史资料离线复核与只读工具）；S1–S5 NOT STARTED；没有真机联网测试通过记录。详情见 [S0 基线](evidence/s0-baseline.md) 和 [工具测试](evidence/s0-tooling-tests.md)。  
...
| S1-02 | PolicyCore 类型、first-match 参考解释器、CIDR 编译与能力检查 | T-P01–P03、P07–P10 |
...
退出条件：声明范围内的 DNS-derived 行为与失效处理正确；能在 UI 中展示而不是隐藏无法覆盖的情况。任意公网后缀自动发现和严格域名隔离不是本阶段任务。
...
新依赖、数据面机制、默认出口、DNS 隐私行为或保证等级变化必须先更新 ADR。下一项实际执行为补齐 S0-01 本机场景，并运行 [S0 只读工具](../tools/s0/README.md) 完成 S0-02 当前基线；旧快照已有离线复核，不必重复上传。不要跳过门槛制作完整 UI 或 Fork 整个参考应用。
