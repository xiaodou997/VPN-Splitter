...
struct VPNSplitterApp: App {
    ...
    @StateObject private var managed = ManagedConfigurationModel.shared
    @NSApplicationDelegateAdaptor(ManagedAppDelegate.self) private var appDelegate
    var body: some Scene {
        WindowGroup("VPN-Splitter · S1") {
            TabView {
                ManagedConfigurationView().disabled(model.busy).tabItem { Text("正式配置 / 交付") }
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("只有日志出现 S1_PROVIDER_REACHED，才证明进入了 Provider。连接失败本身不算通过。")
                        .font(.footnote)
                }.padding(24).frame(minWidth: 720, minHeight: 430).disabled(managed.busy).tabItem { Text("S1 签名测试") }
            }
        }
    }
}
